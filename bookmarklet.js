...
NS_LecteurBD.initialisation = function() {
    ...
    NS.affichage = function() {
        ...
        for (var i = 3; i < NS.list.length; i++) {
            if (~NS.list[i].className.indexOf('dir')) continue;
            var ext = NS.list[i].href.split('.');
            var imgType = ext[ext.length - 1].toLowerCase();
            if (!~allowedTypes.indexOf(imgType)) continue;
    ...
    NS.styleImage = (function() {
        ...
        styl.id = 'styleImage';
        styl.innerHTML = '.image {max-width: 85%;}';
    ...
    NS.maxHeigh = (function() {
        ...
        var txt = document.createTextNode('hauteur limit%C3%A9e %C3%A0 100%.');
    ...
    NS.largeur = (function() { NS.control.appendChild(document.createElement('br')); var txt = document.createElement('span');
        txt.innerHTML = 'largeur:';
        txt.style.paddingLeft = '5px';
        NS.control.appendChild(txt); var selectLargeur = document.createElement('select');
        selectLargeur.id = 'largeur';
        selectLargeur.innerHTML = '\limit%C3%A9e %C3%A0</option>\forc%C3%A9e %C3%A0</option>\auto</option>';
        NS.control.appendChild(selectLargeur);
        selectLargeur.onchange = function() { if (this.value) { NS.type = this.value;
                NS.moins.disabled = NS.widthImage.disabled = NS.plus.disabled = NS.cent.disabled = false;
                NS.setWidth(); } else { NS.styleImage.innerHTML = '.image{width:auto}';
                NS.moins.disabled = NS.widthImage.disabled = NS.plus.disabled = NS.cent.disabled = true; } }; return selectLargeur; })();
    NS.zoom = (function() { NS.control.appendChild(document.createElement('br')); var divZoom = document.createElement('div');
        divZoom.marginTop = '2px';
        NS.control.appendChild(divZoom); return divZoom; })();
    NS.moins = (function() { var moins = document.createElement('button');
        moins.id = 'moins';
        moins.innerHTML = '-5';
        NS.zoom.appendChild(moins);
        moins.onclick = function() { if (NS.percent <= .5) return false;
            NS.percent -= NS.percent > 5 ? (NS.percent % 5) || 5 : (NS.percent % .5) || .5;
            NS.setWidth(); }; return moins; })();
    NS.widthImage = (function() { var val = document.createElement('input');
        val.id = 'widthImage';
        val.type = 'number';
        val.value = 85;
        val.min = 0;
        val.style.width = '40px';
        NS.zoom.appendChild(val); var txt = document.createTextNode('%');
        NS.zoom.appendChild(txt);
        val.onchange = function() { if (this.value) NS.percent = parseFloat(this.value);
            NS.setWidth(); }; return val; })();
    NS.plus = (function() { var plus = document.createElement('button');
        plus.id = 'plus';
        plus.innerHTML = '+5';
        NS.zoom.appendChild(plus);
        plus.onclick = function() { NS.percent += NS.percent >= 5 ? 5 - NS.percent % 5 : .5 - NS.percent % .5;
            NS.setWidth(); }; return plus; })();
    NS.cent = (function() { var cent = document.createElement('button');
        cent.id = 'cent';
        cent.innerHTML = '100%';
        NS.zoom.appendChild(cent);
        cent.onclick = function() { NS.percent = 100;
            NS.setWidth(); }; return cent; })();
    NS.lecteur = (function() { var divLecteur = document.createElement('div');
        divLecteur.id = 'lecteur';
        divLecteur.style.textAlign = 'center';
        document.body.appendChild(divLecteur); return divLecteur; })();
    window.onmousemove = function(a) { if (a.clientX == NS.omm.x && a.clientY == NS.omm.y) return;
        clearTimeout(NS.omm.to);
        document.body.style.cursor = '';
        NS.omm = { to: setTimeout(function() { document.body.style.cursor = 'none'; }, 5000), x: a.clientX, y: a.clientY } };
    NS.affichage();
};
if (/file:\//.test(document.URL))NS_LecteurBD.initialisation();
